        ...
        async function saveDeal() {
            const dealId = document.getElementById('dealId') ? document.getElementById('dealId').value : '';
            const dealName = document.getElementById('dealName') ? document.getElementById('dealName').value : '';
            const salesName = document.getElementById('salesName') ? document.getElementById('salesName').value : '';
            
            // Ambil beforeDiscount sebagai input utama
            const beforeDiscountInput = document.getElementById('beforeDiscount');
            const beforeDiscountRaw = beforeDiscountInput ? beforeDiscountInput.value.replace(/[^0-9]/g, '') : '0';
            const beforeDiscount = parseFloat(beforeDiscountRaw) || 0;
            
            const discountInput = document.getElementById('discount');
            const discount = discountInput ? parseFloat(discountInput.value) || 0 : 0;
            
            // Hitung value dari beforeDiscount dan discount
            let value = beforeDiscount;
            if (discount > 0 && discount <= 100) {
                value = beforeDiscount * (1 - (discount / 100));
            }
            
            const newPackageInput = document.getElementById('newPackage');
            let packageType = newPackageInput ? newPackageInput.value.trim() : '';
            if (!packageType) {
                const packageSelect = document.getElementById('package');
                packageType = packageSelect ? packageSelect.value : '';
            }
            
            // Tambahkan package baru ke uniquePackages jika belum ada
            if (packageType && !uniquePackages.has(packageType)) {
                uniquePackages.add(packageType);
                await saveDropdownOptions();
                updateDropdownOptions();
            }
            
            const productList = document.getElementById('productList');
            const collectedProducts = new Set();
            
            if (productList) {
                const productInputs = productList.querySelectorAll('select, input[type="text"]');
                for (let i = 0; i < productInputs.length; i += 2) {
                    const selectElement = productInputs[i];
                    const textInputElement = productInputs[i + 1];
            
                    let productValue = '';
                    if (textInputElement && textInputElement.value.trim() !== '') {
                        productValue = textInputElement.value.trim();
                    } else if (selectElement && selectElement.value !== '') {
                        productValue = selectElement.value;
                    }
            
                    if (productValue) {
                        collectedProducts.add(productValue);
                    }
                }
            }
            const productsToSave = Array.from(collectedProducts);
        
            const newFacilityInput = document.getElementById('newFacility');
            let facility = newFacilityInput ? newFacilityInput.value.trim() : '';
            if (!facility) {
                facility = facilitySelect ? facilitySelect.value : '';
            }
            
            // Tambahkan facility baru ke uniqueFacilities jika belum ada
            if (facility && !uniqueFacilities.has(facility)) {
            ...
            const newOwnerInput = document.getElementById('newOwner');
            let owner = newOwnerInput ? newOwnerInput.value.trim() : '';
            if (!owner) {
                const ownerSelect = document.getElementById('owner');
                owner = ownerSelect ? ownerSelect.value : '';
            }
            
            // Tambahkan owner baru ke uniqueOwners jika belum ada
            ...
            let consultant = consultantHiddenInput ? consultantHiddenInput.value.trim() : '';
            if (consultant && !uniqueConsultants.has(consultant)) {
                uniqueConsultants.add(consultant);
                console.log(`New consultant "${consultant}" added to local list.`);
            }
        
            const contractorList = document.getElementById('contractorList');
            const collectedContractors = new Set();
            
            if (contractorList) {
                const contractorInputs = contractorList.querySelectorAll('select, input[type="text"]');
            ...
            const contractorsToSave = Array.from(collectedContractors);
        
            const newPicInput = document.getElementById('newPic');
            let pic = newPicInput ? newPicInput.value.trim() : '';
            if (!pic) {
                const picSelect = document.getElementById('pic');
                pic = picSelect ? picSelect.value : '';
            }
            
            // Tambahkan pic baru ke uniquePICs jika belum ada
            if (pic && !uniquePICs.has(pic)) {
                uniquePICs.add(pic);
                await saveDropdownOptions();
                updateDropdownOptions();
            }
            ...
            const planPOSelect = document.getElementById('planPO');
            const planPO = planPOSelect ? planPOSelect.value : '';
            
            const stageSelect = document.getElementById('stage');
            const stage = stageSelect ? stageSelect.value : DEFAULT_STAGE;
            
            const prioritySelect = document.getElementById('priority');
            const priority = prioritySelect ? prioritySelect.value : 'Priority';
            
            const remarksTextarea = document.getElementById('remarks');
            const remarks = remarksTextarea ? remarksTextarea.value : '';
            
            const progressValueInput = document.getElementById('progressValue');
            const progress = progressValueInput ? parseInt(progressValueInput.value) || 0 : 0;
            ...
            if (!beforeDiscount) {
                showToast("Sebelum Diskon (IDR) harus diisi.", 3000);
                return;
            }
        
            if (dealId && currentUserRole === 'user') {
                const deal = deals.find(d => d.id === dealId);
                if (deal && !canUserEditDeal(deal)) {
                    showToast("Anda tidak memiliki izin untuk mengedit deal ini.", 3000);
                    return;
                }
            }
            ...
            try {
                if (dealId) {
                    ...
                    await activitiesCollection.add({
                        message: `Deal "${dealName}" diperbarui oleh ${auth.currentUser.email}. (stage: ${stage})`,
                        timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                        userEmail: auth.currentUser.email,
                        read: false
                    });
                    ...
                    await activitiesCollection.add({
                        message: `Deal "${dealName}" ditambahkan oleh ${auth.currentUser.email}. (stage: ${stage})`,
                        timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                        userEmail: auth.currentUser.email,
                        read: false
                    });
                ...
                // Reset active sales per deal untuk deal yang baru diubah
                delete activeSalesPerDeal[dealName.toLowerCase()];
        ...
        // Fungsi untuk menyiapkan edit deal
        ...
        function openDealDetailModal(dealId) {
            ...
            document.getElementById('detailProduct').textContent = Array.isArray(deal.product) ? deal.product.join(', ') : (deal.product || '-');
            ...
            document.getElementById('detailContractor').textContent = Array.isArray(deal.contractor) ? deal.contractor.join(', ') : (deal.contractor || '-');
            ...
            document.getElementById('detailProgress').textContent = deal.progress ? `${deal.progress}%` : '-';
            document.getElementById('detailRemarks').textContent = deal.remarks || '-';
            
            // Load komentar
            loadComments(dealId).then(comments => {
                renderComments(comments, 'detailCommentsList');
            });
        ...
        function confirmDeleteDeal(dealId, dealName) {
            if (!canUserEditDeal(deals.find(d => d.id === dealId))) {
                showToast("Anda tidak memiliki izin untuk menghapus deal ini", 3000);
                return;
            }
            ...
            // Set data untuk delete
            document.getElementById('confirmDeleteBtn').dataset.id = dealId;
        }
        ...
        async function deleteDeal() {
            const dealId = document.getElementById('confirmDeleteBtn').dataset.id;
            const deal = deals.find(d => d.id === dealId);
            ...
            try {
                ...
                await deletedDealsCollection.add(dealData);
                
                // Hapus dari collection utama
                await dealsCollection.doc(dealId).delete();
                
                showToast(`Deal "${deal.dealName}" berhasil dipindahkan ke Recycle Bin!`, 2000);
                
                // Log aktivitas
                await activitiesCollection.add({
                    message: `Deal "${deal.dealName}" dihapus oleh ${auth.currentUser.email}.`,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                    userEmail: auth.currentUser.email,
                    read: false
                });
                
                // Reset active sales per deal untuk deal yang dihapus
                delete activeSalesPerDeal[deal.dealName.toLowerCase()];
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log("Sales Pipeline Manager initialized");
        });
    </
